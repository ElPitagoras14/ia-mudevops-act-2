...
def get_book(book_id: uuid.UUID, _: dict = Depends(get_current_user)):
    rows = execute_query(
...
def update_book(book_id: uuid.UUID, body: BookUpdate, _: dict = admin_required):
    ...
    values = list(fields.values()) + [str(book_id)]
    book = execute_insert(
...
def delete_book(book_id: uuid.UUID, _: dict = admin_required):
    from app.db import get_connection
    conn = get_connection()
    cursor = conn.cursor()
    try:
        cursor.execute("DELETE FROM books WHERE id = %s", (str(book_id),))
